    ...
    * raise ``RuntimeError`` if the playbook fails or returns unparseable
    ...
    def _leer(
        ...
        *,
        ...
        logger.info("%s: run %d command(s) on device=%s (partial_ok=%s)", type(self).__name__, len(commands), device.name, partial_ok)
        # tolerate_command_errors: extravar leído por el playbook de cada
        # vendor. Cuando True selecciona una task "tolerant" (failed_when:
        # false) para que ios_command / cli_command no marquen la task
        # como fallida cuando un comando individual devuelve "% Invalid
        # input" (Cisco) o "Error: Unrecognized command" (Huawei) -- así
        # el stdout resultante contiene la respuesta de CADA comando,
