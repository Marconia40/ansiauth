...
> `interface`/`vlan_id` en el body) — 1 fila de config por device. El `GET /`
> agrupa lo relacionado en sub-objetos (`snmp`, `ntp`, `dns`, `logging`) en
> vez de todo plano — pedido del usuario tras ver la respuesta original.
> `ntp.servers`/`dns.servers`/`logging.servers`/`logging.level`/
> `snmp.version` se extraen internamente del `running-config` del device
> (mismo comando ya leído, sin disparar uno nuevo). `ntp.servers`/
> `dns.servers`/`logging.servers` son **listas** (puede haber más de 1
> configurado, ej. varios `ip name-server`) — no solo el primer valor.
> `snmp.trap_hosts` (lista) en Cisco son los destinos reales de traps
> (`snmp-server host`, puede haber más de 1 línea). En Huawei no hay
> trap-host leíble (`snmp-agent target-host` no tiene lectura implementada)
> — en cambio se resuelve cruzando `snmp-agent acl {nombre}` (la ACL atada
> al agente SNMP) contra `acls`, tomando TODOS los `permit source` de esa
> ACL — confirmado en vivo que la primera IP coincide con el `trap_host`
> real de Cisco en este par de devices (`172.19.19.46`), y que la ACL trae
> además una 2da IP (`200.16.16.13`) que antes se perdía mostrando solo el
> primer match.
> Solo `snmp.community`/`snmp.permission` siguen en `null` en Huawei — VRP
> la guarda cifrada, no hay forma de revertirla del lado del parser (en
> Cisco sí funciona, la guarda en texto plano). `acls` trae el nombre/
> tipo/reglas de cada ACL (línea cruda tal cual el device, sin
> re-estructurar).
> El dump completo y sin procesar del `running-config` (100% real, como
> lista de líneas) NO viene en este `GET /` — vive en su propio `GET
> /global-config/running-config` (ver 1.1b), separado a pedido del usuario
> por ser lo más pesado de la respuesta.
...
RF-GLOBAL-01 pedía "consultar configuración general **y/o** versión" — son 3
cosas conceptualmente distintas (ajustes puntuales / dump completo / versión),
así que quedaron en 3 endpoints separados.

### 1.1 Ver los ajustes configurados (GET, cache-first)
...
Lista todo lo que se puede configurar desde esta app: `hostname`, `snmp`
(`enabled`/`version`/`community`/`permission`/`trap_hosts` — lista), `ntp`/`dns`/
`logging` (`servers` — lista, `level` en logging), `routes`, `acls`
(`name`/`type`/`rules`, reglas crudas de cada ACL). NO trae el dump
completo de config ni la versión del device — esos viven en sus propios
endpoints (1.1b/1.1c), son lo más pesado de la respuesta y conceptualmente
otra cosa.

### 1.1b Ver el running-config completo (GET, mismo cache)

```bash
curl -sS "${AUTH[@]}" "$BASE/devices/$CISCO/global-config/running-config" | python3 -m json.tool
curl -sS "${AUTH[@]}" "$BASE/devices/$HUAWEI/global-config/running-config" | python3 -m json.tool
```

`running_config` es la config real del device como **lista de líneas** (no 1
string con `\n` adentro) — ya viene sin las líneas de metadata que trae
`show running-config`/`display current-configuration` de arranque
("Building configuration...", "Current configuration : N bytes", etc.).

### 1.1c Ver versión del device (GET, mismo cache)

```bash
curl -sS "${AUTH[@]}" "$BASE/devices/$CISCO/global-config/version" | python3 -m json.tool
curl -sS "${AUTH[@]}" "$BASE/devices/$HUAWEI/global-config/version" | python3 -m json.tool
```

No dispara una lectura nueva (mismo `show version`/`display version` que ya
se guarda en el sync de `GET /`). `software_version`/`model`/`uptime` son
best-effort (el formato de este comando varía mucho incluso dentro del
mismo vendor/familia). `raw`/`serial_number` se sacaron de la respuesta a
pedido del usuario -- ya no se exponen.
...
> request, usa la que ya esté configurada en el device. Una vez aplicado,
> `snmp.trap_hosts` en el `GET /` (sección 1.1) lo confirma.
>
...
> vieja al instante. La respuesta viene **parseada a filas** (`entries`),
> no como texto crudo — confirmado en vivo contra f3r9s1/f3r9s2 (los totales
> parseados matchean el "Total:N"/"Total items displayed = N" que reporta
> el propio device). Campos por fila:
> - ARP: `ip`, `mac`, `interface`, `vlan` (solo Huawei, viene de una 2da
>   línea de la tabla; en Cisco siempre `null`), `type`, `age`.
> - MAC: `mac`, `vlan`, `interface`, `type`.
>
> `include` sigue siendo opcional, se pasa tal cual al `| include` del
> propio device ANTES de parsear (para acotar la tabla del lado del
> device). Charset permitido: alfanumérico + `:./_-`, sin espacios ni pipes
> ni saltos de línea (422 si no cumple — es lo que viaja dentro de una línea
> de comando sobre la sesión SSH, no se puede dejar pasar cualquier cosa).
> Pide rol `observer` (no `admin`, es solo lectura).
