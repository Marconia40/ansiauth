"""add_interfaces_virtuales

RF-INTERV-01 a 09 — SVIs (interface Vlan{n} en Cisco, Vlanif{n} en Huawei).

* ``device_interfaces_virtuales`` -- PK compuesta (vlan_id, device), mismo
...
* ``devices.interfaces_virtuales_synced_at`` / ``_sync_error`` -- mismo par
  de metadata cache-first que ya existe para vlans/ports (front-data).

Revision ID: k5msp10_interfaces_virtuales
...
revision: str = "k5msp10_interfaces_virtuales"
...
def upgrade() -> None:
    ...
    op.add_column(
        "devices",
        sa.Column("interfaces_virtuales_synced_at", sa.DateTime(timezone=True), nullable=True),
    )
    op.add_column(
        "devices",
        sa.Column("interfaces_virtuales_sync_error", sa.Text(), nullable=True),
    )


def downgrade() -> None:
    op.drop_column("devices", "interfaces_virtuales_sync_error")
    op.drop_column("devices", "interfaces_virtuales_synced_at")
    op.drop_table("device_interfaces_virtuales")
